...
class CategoryViewer extends ContextSource {
	 ...
	 * @param array $query
	 */
	function CategoryViewer( $title, IContextSource $context, $from = [], $until = [], $query = []) {
		$this->title = $title;
	...
	public function getHTML() {
		...
		$this->showGallery = $this->getConfig()->get( 'CategoryMagicGallery' ) && !$this->getOutput()->mNoGallery;
	...
	function addImage(Title $title, $sortkey, $pageLength, $isRedirect = false) {
		...
		if ($this->showGallery) {
			...
			$this->imgsNoGallery_start_char[] = $wgContLang->convert($this->collation->getFirstLetter($sortkey));
		}
	}
	...
	function doCategoryQuery() {
		...
		foreach (['page', 'subcat', 'file'] as $type) {
			...
			$res = $dbr->select(
				...
				['page_id', 'page_title', 'page_namespace', 'page_len', 'page_is_redirect', 'cl_sortkey', 'cat_id', 'cat_title', 
					'cat_subcats', 'cat_pages', 'cat_files', 'cl_sortkey_prefix', 'cl_collation'],
				array_merge(['cl_to' => $this->title->getDBkey()], $extraConds),
				__METHOD__,
				['USE INDEX' => ['categorylinks' => 'cl_sortkey'], 'LIMIT' => $this->limit + 1, 
					'ORDER BY' => $this->flip[$type] ? 'cl_sortkey DESC' : 'cl_sortkey', ],
				[
					'categorylinks' => ['INNER JOIN', 'cl_from = page_id'],
					'category' => ['LEFT JOIN', ['cat_title = page_title', 'page_namespace' => NS_CATEGORY]]
				]
			);
	...
	function getCategoryTop() {
		...
		return $r === '' ? $r : "<br style=\"clear:both;\"/>\n" . $r;
	}
	...
	function getSubcategorySection() {
		...
		if ( $rescnt > 0 ) {
			...
			$r .= "\n";
			$r .= Html::element('h2', [], $this->msg('subcategories' )->text());
			$r .= "\n";
			$r .= $countmsg;
	...
	function getPagesSection() {
		...
		$dbcnt = $this->cat->getPageCount() - $this->cat->getSubcatCount() - $this->cat->getFileCount();
		...
		if ($rescnt > 0) {
			...
			$r .= "\n";
			$r .= Html::element('h2', [], $this->msg('category_header', $ti )->text());
			$r .= "\n";
			$r .= $countmsg;
	...
	function formatList($articles, $articles_start_char, $cutoff = 6) {
		...
		$attribs = ['lang' => $pageLang->getHtmlCode(), 'dir' => $pageLang->getDir(), 'class' => 'mw-content-' . $pageLang->getDir()];
	...
	private function pagingLinks($first, $last, $type = '') {
		...
		if ($first != '') {
			...
			$prevLink = Linker::linkKnown($this->addFragmentToTitle($this->title, $type), $prevLink, [], $prevQuery);
		}
		...
		if ($last != '') {
			...
			$nextLink = Linker::linkKnown($this->addFragmentToTitle($this->title, $type), $nextLink, [], $lastQuery);
		}
	...
	private function addFragmentToTitle($title, $section) {
		switch ( $section ) {
			...
			case 'file':
				...
				break;
			default:
				throw new MWException( __METHOD__ . " Invalid section $section." );
		}

		return Title::makeTitle($title->getNamespace(), $title->getDBkey(), $fragment);
	}
